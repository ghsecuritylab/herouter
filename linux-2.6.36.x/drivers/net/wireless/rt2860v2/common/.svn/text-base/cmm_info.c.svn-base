...
extern char g_szWpsMacString[256];
extern char g_szHideSpecialSsidMacString[256];
extern char g_szGuestSsidMacString[256];
extern char g_szHideSpecialSSid[128];
...
/* +++ added by Red@Ralink, 2009/09/30 */
VOID RTMPIoctlGetFilterMacTableStaInfo(
	IN PRTMP_ADAPTER pAd, 
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	INT i;
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: >>>>>> DATA TYPE(%d) >>>>>>>\n", __FUNCTION__, *wrq->u.data.pointer));
	if ( 1 == (*wrq->u.data.pointer) )
	{
		pSrcData = g_szWpsMacString;
		wrq->u.data.length  = sizeof(g_szWpsMacString);
	}
	else if ( 2 == (*wrq->u.data.pointer) )
	{
		pSrcData = g_szHideSpecialSsidMacString;
		wrq->u.data.length  = sizeof(g_szHideSpecialSsidMacString);
	}
	else if ( 3 == (*wrq->u.data.pointer) )
	{
		pSrcData = g_szGuestSsidMacString;
		wrq->u.data.length  = sizeof(g_szGuestSsidMacString);
	}
	else
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s: >>>>>> DATA TYPE(%d) >>>>>>>\n", __FUNCTION__, *wrq->u.data.pointer));
		return;
	}
	if (copy_to_user(wrq->u.data.pointer, pSrcData, wrq->u.data.length))
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s: copy_to_user() fail\n", __FUNCTION__));
	}
}

VOID RTMPIoctlSetFilterHideSsid(
	IN PRTMP_ADAPTER pAd, 
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	INT i;
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: >>>>>> DATA TYPE(%d) >>>>>>>\n", __FUNCTION__, *wrq->u.data.pointer));
	if ( 0x0 != (*wrq->u.data.pointer) )
	{
		copy_from_user(g_szHideSpecialSSid, wrq->u.data.pointer, sizeof(g_szHideSpecialSSid));
		DBGPRINT(RT_DEBUG_ERROR, ("%s: copy_from_user(%s)\n", __FUNCTION__, g_szHideSpecialSSid));
	}
}
