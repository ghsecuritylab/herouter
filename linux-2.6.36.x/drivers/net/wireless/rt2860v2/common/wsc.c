...
//char g_szWpsMacString[256] = {"d0:7b:b5:0f:5c:9d"};//debug
//char g_szWpsMacString[256] = {"d0:7b:b5:0f:5c:9d,d0:7a:b5:0f:5c:9d"};//debug
char g_szWpsMacString[256] = {0x0};
...
/* DARE, zhuzhh, 20140521, add start */
	...
	sprintf(szBufTemp, "%02x:%02x:%02x:%02x:%02x:%02x", PRINT_MAC(MacAddr));

	pStart = g_szWpsMacString;
	...
	memset(g_szWpsMacString, 0x0, sizeof(szBufTemp));
	strcpy(g_szWpsMacString, szBufTemp);
	DBGPRINT(RT_DEBUG_TRACE, ("WscEAPAction: ########## %s ##########\n",g_szWpsMacString));
/* DARE, zhuzhh, 20140521, add end */

	DBGPRINT(RT_DEBUG_ERROR, ("WscEAPAction: Addr: %02x:%02x:%02x:%02x:%02x:%02x, MsgType: 0x%02X, bUPnPMsg: %s\n",
