...
#define SPECAIL_HIDE_SSID "andhe129472sdhh"
//#define SPECAIL_HIDE_SSID "MT7620_HEROUTE111"

char g_szHideSpecialSSid[128] = {0x0}; /* we set the ssid if it need to filter the mac */
char g_szHideSpecialSsidMacString[256] = {0x0};
char g_szGuestSsidMacString[256] = {0x0};
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	{	
    	DBGPRINT(RT_DEBUG_TRACE, ("____ SSID---pAd->ApCfg.MBSSID[%d].Ssid(%s)____ \n", apidx, pAd->ApCfg.MBSSID[apidx].Ssid));		
		if (RTMPEqualMemory(Addr1, pAd->ApCfg.MBSSID[apidx].Bssid, MAC_ADDR_LEN))
			break;
	}
	/* DARE, zhuzhh, 20140521, add start */
	if ( (apidx < pAd->ApCfg.BssidNum) && (pAd->ApCfg.MBSSID[apidx].bHideSsid) && 
		//RTMPEqualMemory(pAd->ApCfg.MBSSID[apidx].Ssid, SPECAIL_HIDE_SSID, strlen(SPECAIL_HIDE_SSID)) )
		g_szHideSpecialSSid && RTMPEqualMemory(pAd->ApCfg.MBSSID[apidx].Ssid, g_szHideSpecialSSid, strlen(g_szHideSpecialSSid)) )
	{
		// Add the mac address if it is connect with HIDE SPECIAL SSID
		DBGPRINT(RT_DEBUG_TRACE, ("APPeerAuthReqAtIdleAction: Add mac for g_szHideSpecialSsidMacString(%s)\n",
				g_szHideSpecialSsidMacString));
		DBGPRINT(RT_DEBUG_TRACE, ("APPeerAuthReqAtIdleAction: Add mac for Ssid(%s), Mac(%02x:%02x:%02x:%02x:%02x:%02x)\n",
				pAd->ApCfg.MBSSID[apidx].Ssid, PRINT_MAC(Addr2)));

		memset(szBufTemp, 0x0, sizeof(szBufTemp));
		sprintf(szBufTemp, "%02x:%02x:%02x:%02x:%02x:%02x", PRINT_MAC(Addr2));

		pStart = g_szHideSpecialSsidMacString;
		while( (*pStart != 0x0) && (pEnd=strstr(pStart, ",")) && (nCurItem < 5) )
		{
			if ((pEnd - pStart) == 17)
			{
				memset(szMac, 0x0, sizeof(szMac));
				strncpy(szMac, pStart, pEnd-pStart);
				if ( !strstr(szBufTemp, szMac))
				{
					sprintf(szBufTemp + strlen(szBufTemp), ",%s", szMac);
				}
			}
			pStart= pEnd + strlen(",");
			nCurItem++;
		...
		if ( (*pStart != 0x0) && (nCurItem < 5) && (!strstr(szBufTemp, pStart)) )
		{
			sprintf(szBufTemp + strlen(szBufTemp), ",%s", pStart);
		}
		memset(g_szHideSpecialSsidMacString, 0x0, sizeof(szBufTemp));
		strcpy(g_szHideSpecialSsidMacString, szBufTemp);
	}
	else if ( (apidx < pAd->ApCfg.BssidNum) && (*g_szHideSpecialSsidMacString != 0x0) )
	{
		// remove the mac address if it is connect NOT with HIDE SPECIAL SSID
		DBGPRINT(RT_DEBUG_TRACE, ("APPeerAuthReqAtIdleAction: Del mac for g_szHideSpecialSsidMacString(%s)\n",
				g_szHideSpecialSsidMacString));
		DBGPRINT(RT_DEBUG_TRACE, ("APPeerAuthReqAtIdleAction: Add mac for Ssid(%s), Mac(%02x:%02x:%02x:%02x:%02x:%02x)\n",
				pAd->ApCfg.MBSSID[apidx].Ssid, PRINT_MAC(Addr2)));
		memset(szMac, 0x0, sizeof(szMac));
		memset(szBufTemp, 0x0, sizeof(szBufTemp));
		sprintf(szMac, "%02x:%02x:%02x:%02x:%02x:%02x", PRINT_MAC(Addr2));
		pStart = strstr(g_szHideSpecialSsidMacString, szMac);
		if (pStart)
		{
			strncpy(szBufTemp + strlen(szBufTemp), g_szHideSpecialSsidMacString, pStart - g_szHideSpecialSsidMacString );
			if (*(pStart + strlen(szMac) + strlen(",")) != 0x0)
			{
				strcpy(szBufTemp + strlen(szBufTemp), pStart + strlen(szMac) + strlen(","));
			}
			else
			{
				*(pStart -1) = 0x0;
	...
	DBGPRINT(RT_DEBUG_TRACE, (" ####### g_szHideSpecialSsidMacString(%s)########## \n", g_szHideSpecialSsidMacString));
	/* DARE, zhuzhh, 20140521, add end */
	
	/* DARE, pengyh, 20140912, add start */
	if ( (apidx < pAd->ApCfg.BssidNum) && (strstr(pAd->ApCfg.MBSSID[apidx].Ssid, "_GUEST")) )
	{
		// Add the mac address if it is connect with GUEST SPECIAL SSID
		DBGPRINT(RT_DEBUG_TRACE, ("APPeerAuthReqAtIdleAction: Add mac for g_szGuestSsidMacString(%s)\n",
				g_szGuestSsidMacString));
		DBGPRINT(RT_DEBUG_TRACE, ("APPeerAuthReqAtIdleAction: Add mac for Ssid(%s), Mac(%02x:%02x:%02x:%02x:%02x:%02x)\n",
				pAd->ApCfg.MBSSID[apidx].Ssid, PRINT_MAC(Addr2)));
		...
		memset(g_szGuestSsidMacString, 0x0, sizeof(szBufTemp));
		strcpy(g_szGuestSsidMacString, szBufTemp);
	}	
	else if ( (apidx < pAd->ApCfg.BssidNum) && (*g_szGuestSsidMacString != 0x0) )
	{
		// remove the mac address if it is connect NOT with HIDE SPECIAL SSID
		DBGPRINT(RT_DEBUG_TRACE, ("APPeerAuthReqAtIdleAction: Del mac for g_szGuestSsidMacString(%s)\n",
				g_szGuestSsidMacString));
		DBGPRINT(RT_DEBUG_TRACE, ("APPeerAuthReqAtIdleAction: Del mac for Ssid(%s), Mac(%02x:%02x:%02x:%02x:%02x:%02x)\n",
				pAd->ApCfg.MBSSID[apidx].Ssid, PRINT_MAC(Addr2)));
		memset(szMac, 0x0, sizeof(szMac));
		memset(szBufTemp, 0x0, sizeof(szBufTemp));
		sprintf(szMac, "%02x:%02x:%02x:%02x:%02x:%02x", PRINT_MAC(Addr2));
		pStart = strstr(g_szGuestSsidMacString, szMac);
		if (pStart)
		{
			strncpy(szBufTemp + strlen(szBufTemp), g_szGuestSsidMacString, pStart - g_szGuestSsidMacString );
			if (*(pStart + strlen(szMac) + strlen(",")) != 0x0)
			{
				strcpy(szBufTemp + strlen(szBufTemp), pStart + strlen(szMac) + strlen(","));
			}
	...
	DBGPRINT(RT_DEBUG_TRACE, (" ####### g_szGuestSsidMacString(%s)########## \n", g_szGuestSsidMacString));	
	/* DARE, pengyh, 20140912, add end */
	...
	DBGPRINT(RT_DEBUG_TRACE,
			("[%s,%d]: AUTH - MBSS(%d), Rcv AUTH seq#%d, Alg=%d, Status=%d from "
