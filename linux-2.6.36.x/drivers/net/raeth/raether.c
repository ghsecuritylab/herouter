...
//add fangan for save WAN port state
static void savePortState(int portNum,int portState)
{
	  
    if(portNum > -1 && portNum < 3)
    {
    	  if(portState == 0)
    		    ethPortStatus[portNum] = 'd';  
    		else if(portState == 1)
    		    ethPortStatus[portNum] = 'u'; 	
    }
/*
    struct file *fp;  
    mm_segment_t fs;  
    ...
    if(portNum == 0)
    {
        strcpy(fileName,"/var/wanPort_state");
    }
    else if(portNum == 1)
    {
        strcpy(fileName,"/var/lan1Port_state");
    }
    else if(portNum == 2)
    {
        strcpy(fileName,"/var/lan2Port_state");
		}
		
    sprintf(inputBuf,"%d&%d",portNum,portState);
    ...
    fs = get_fs();  
    set_fs(KERNEL_DS);  
    vfs_write(fp, "testtest1", sizeof(inputBuf), &pos);  
    filp_close(fp, NULL);  
    set_fs(fs);  
*/  
}
...
static void esw_link_status_changed(int port_no, void *dev_id)
{
	...
	if(port_no == 0 || port_no == 1 || port_no == 2)
	{
	    savePortState(port_no,1);
	}
...
#define _HQOS_REG(x)	(*((volatile u32 *)(RALINK_FRAME_ENGINE_BASE + QDMA_RELATED + x)))
		...
		case RAETH_PORTSTATE_READ:
			copy_to_user(ifr->ifr_data, ethPortStatus, sizeof(ethPortStatus));
			break;
