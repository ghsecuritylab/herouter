			      ...
			      
{
  ...
  struct server *serv;
  unsigned short flags = 0,autojump = 0;
  
  const char *needWizard = nvram_bufget(RT2860_NVRAM, "needWizard");
  ...
  //the broadlink device maybe send a lots of packets to the ip of 192.168.8.1,so ignone it.
  if((strcmp(needWizard,"2") != 0) && (strstr(qdomain,".broadlink.") == NULL))
  {
      autojump = 1;
  	  my_syslog(LOG_WARNING, _("start auto jump domain:%s"), qdomain);
  }

  /*add dare fangan,strcmp(needWizard,"0") == 0,if need wizard then redirect to login page*/
	if ((autojump == 1) ||
		  (namelen >= domainlen &&	\
