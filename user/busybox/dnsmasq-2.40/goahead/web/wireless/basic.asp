...
var ht_ldpc = '<% getCfgZero(1, "HT_LDPC"); %>';
var vht_bw = '<% getCfgZero(1, "VHT_BW"); %>';
var vht_stbc = '<% getCfgZero(1, "VHT_STBC"); %>';
var vht_sgi = '<% getCfgZero(1, "VHT_SGI"); %>';
var vht_bw_signal = '<% getCfgZero(1, "VHT_BW_SIGNAL"); %>';
...
function ChOnChange()
{
	if (document.wireless_basic.n_bandwidth[1].checked == true)
	{
		...
		else if ((1*w_mode == 9) || (1*w_mode == 6) || (1*w_mode >= 14))
		{
			if (document.wireless_basic.n_bandwidth[1].checked == true)
			{
				document.getElementById("extension_channel").style.visibility = "visible";
...
function changeVHTBW()
{
	...
	if (1*w_mode >= 12)
	{
		...
		document.getElementById("div_vht_bw").style.display = StylDispOn();
		if (document.wireless_basic.n_bandwidth[1].checked == true)
		{
...
function initValue()
{
	...
	if ((1*PhyMode == 8) || (1*PhyMode == 11) || (1*PhyMode == 15))
	{
		if (document.wireless_basic.sz11aChannel.options.selectedIndex == 0)
		{
			document.getElementById("extension_channel").style.visibility = "hidden";
