 ...
 *
 * $Id: //WIFI_SOC/MP/SDK_4_2_0_0/RT288x_SDK/source/user/goahead/src/webs.c#1 $
 */
...
#define UPLOAD_URL "/herouter/AppRequest/UploadIMG/"
#define DOWNLOAD_URL "/herouter/AppRequest/DownloadIMG/"
/******************************** Global Data *********************************/
 ...
 */
	for (i = 0; i <= retries; i++) {
		websListenSock = socketOpenConnection(NULL, port, websAccept, 0, 0);
		if (websListenSock >= 0) {
			break;
		}
		port++;
	...
	ipv6_port = port+1;
	for (i = 0; i <= retries; i++) {
		websListenSock = socketOpenConnection(NULL, ipv6_port, websAccept, 0, 1);
		if (websListenSock >= 0) {
			break;
		}
		ipv6_port++;
 ...
 */
    ...
    system("echo finishedstart > /var/finishstart");

    hertUtil_SetHideSSidForFilter();
...
/*
#define CHECK_PRINT_STR(a) (a)? a : "NULL"

       printf("[%s,%d]: ########websUrlHandlerRequest-START,url:%s,wp->path:%s,query:%s, wp->state(%d)\n",\
			__FUNCTION__, __LINE__, CHECK_PRINT_STR(wp->url),CHECK_PRINT_STR(wp->path),CHECK_PRINT_STR(wp->query), wp->state);
*/
 ...
 */
               ...
               if (text != NULL)
               {
               	  if(strncmp(wp->url,UPLOAD_URL,strlen(UPLOAD_URL)))
               	  {
		                  len = gstrlen(wp->query);
		                  wp->query = brealloc(B_L, wp->query, (len +	gstrlen(text) +
		                     1) * sizeof(char_t));
		               ...
		               {
		                  len = wp->writeBodyLength;
		                  wp->query = brealloc(B_L, wp->query, (len +	nbytes) * sizeof(char_t));
		                  if (wp->query) {
		                     memcpy(&wp->query[len], text,nbytes);
		                  }
		                  wp->writeBodyLength = wp->writeBodyLength + nbytes;
		               }
                   myLen = wp->writeBodyLength;
               }
				}
			} else {
 ...
 */
      isImageUpload = 0;	
      if(strncmp(wp->url,UPLOAD_URL,strlen(UPLOAD_URL)) == 0)
          isImageUpload = 1;	
	    buf[nbytes] = '\0';
 ...
 */
//#ifdef HP_FIX
/*this step can disconnet invalid http request or bad connect,must excute here*/
#if 1
				websDone(wp, 0);
#endif /*HP_FIX*/
 ...
 */
	...
	if (gstrstr(url, CGI_BIN) != NULL) {
		...
		// special case: upload.cgi but not include upload.cgiauto
		if(gstrstr(url,T("upload.cgiauto")) != NULL);
		else if(gstrstr(url, CGI_FIRMWARE_UPLOAD) != NULL){
 ...
 */
		if (gstrcmp(key, T("user-agent")) == 0) {
			...
			if((strstr(wp->userAgent,"Mobile")) || (strstr(wp->userAgent,"SymbianOS")) || (strstr(wp->userAgent,"NOKIA")))
			{
			   	clientip = wp->ipaddr;
			    ismobile = 1;
			}else{
...
void websRedirect(webs_t wp, char_t *url)
{
  ...
  memset(lan_ip,0x0,sizeof(lan_ip));
 ...
 */
    ...
    if ((getIfIp(getLanIfName(), lan_ip)) == -1) {
        printf("can't get lan ip.\n");
        strcpy(lan_ip,"192.168.8.1");  
    }
...
int websWrite(webs_t wp, char_t *fmt, ...)
{
	...
	char_t		*buf;
	int			 rc;
	char     ImgWriteBuf[WEBS_BUFSIZE] = {0};
...
/*handle for app download pictures*/
int WriteImg(webs_t wp, char *zImgBuf)
{
	int			 rc;
	...
	a_assert(zImgBuf);
	if (wp->readBodyLength > 0) {
			rc = websWriteBlock(wp, zImgBuf, wp->readBodyLength);
	}
	return rc;
}
