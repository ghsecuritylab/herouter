 ...
 *
 * $Id: //WIFI_SOC/MP/SDK_4_2_0_0/RT288x_SDK/source/user/goahead/src/handler.c#1 $
 */
...
#define CHECK_PRINT_STR(a) (a)? a : "NULL"
	...
	printf("[%s,%d]: ########websUrlHandlerRequest-START,url:%s,wp->path:%s,query:%s\n",\
			__FUNCTION__, __LINE__, CHECK_PRINT_STR(wp->url),CHECK_PRINT_STR(wp->path),CHECK_PRINT_STR(wp->query));
*/
 ...
 */
	...
	for (i = 0; i < websUrlHandlerMax; i++) {
		...
		if (sp->handler && gstrncmp(sp->urlPrefix, wp->path, sp->len) == 0) {
			...
			//printf("########websUrlHandlerRequest-START,username:%s,password:%s,url:%s,wp->path:%s,query:%s,sp->urlPrefix:%s,sp->webDir:%s\n",\
								//wp->userName,wp->password,wp->url,wp->path,wp->query,sp->urlPrefix,sp->webDir);
			
			if(gstrncmp("/herouter/AppRequest", wp->path, strlen("/herouter/AppRequest")) == 0){
				  websHertAppHandler(wp);
			    return 0;	
			}
