...
*/
 ...
 char *hert_Util_getStrformCmd(char *str)
{
	...
	for(i=0;i<4;i++)
	{
        ...
        int size;
		if((size=fread(szbuf,1,sizeof(szbuf),fp))<20)
		{
			pclose(fp);
			continue;
		}
...
static void getSystemLoading(webs_t wp, char_t *path, char_t *query)
{
	unsigned int mem, cpu, io;

	websWrite(wp, T("HTTP/1.1 200 OK\nContent-type: text/plain\nPragma: no-cache\nCache-Control: no-cache\n\n"));
#if 1
	FILE *pp = popen("top -n 1 | grep Mem:| sed 's/K.*$//g'", "r");
	char buf[128], mem_used[10], mem_total[10], cpu_idle[10], wait_io[10];

	fgets(buf, 128, pp);
	//fprintf(stderr, "%s\n", buf);
	sscanf(buf, "%*s %s", mem_used);
	pclose(pp);
	//fprintf(stderr, "memUsed:%s\n", mem_used);
	pp = popen("cat /proc/meminfo | grep MemTotal: | sed 's/kB//g'", "r");
	fgets(buf, 128, pp);
	pclose(pp);
	sscanf(buf, "%*s %s", mem_total);
	//fprintf(stderr, "%s\n", mem_total);
	mem = (strtol(mem_used, NULL, 10)*100)/strtol(mem_total, NULL, 10);
	pp = popen("top -n 1 | grep CPU:", "r");
	fgets(buf, 128, pp);
	//fprintf(stderr, "%s\n", buf);
	sscanf(buf, "%*s %*s %*s %*s %*s %*s %*s %s %*s %s %*s %*s %*s %*s %*s", cpu_idle, wait_io);
	pclose(pp);
	//fprintf(stderr, "cpuIdle:%s, ioWait:%s\n", cpu_idle, wait_io);
	cpu = strtol(cpu_idle, NULL, 10);
	io = strtol(wait_io, NULL, 10);
	websWrite(wp, T("%d;%d;%d"), cpu, mem, io);
	//fprintf(stderr, "%d %d %d\n", cpu, mem, io);
#else
	int count = strtol(query, NULL, 10);
	srand(time(NULL));
	while (count > 0) {
		mem = rand()%100;
		cpu = rand()%100;
...
static void LoadDefaultSettings(webs_t wp, char_t *path, char_t *query)
{
	...
	if(!hert_Util_getStrformCmd("nvram_get uboot HE_ROUTE_SN"))
	{
	    if((buf=hert_Util_getStrformCmd("nvram_get HE_ROUTE_SN")))
		{
		  sprintf(cmd,"nvram_set uboot HE_ROUTE_SN %s",buf);
		  system(cmd);
				printf("[%s\n]",cmd);
		}
	}
...
#endif/*
		if((buf=hert_Util_getStrformCmd("nvram_get uboot HE_ROUTE_SN")))
		{
		  bzero(cmd,sizeof(cmd));
		  sprintf(cmd,"nvram_set  HE_ROUTE_SN %s",buf);
		  system(cmd);
	      printf("[%s\n]",cmd);
		}
*/
...
static void reboot(webs_t wp, char_t *path, char_t *query)
{
	system("reboot");
}
...
static void hertsyslog(webs_t wp, char_t *path, char_t *query)
{
	FILE *fp = NULL;
	char *log;
	int index=1;
	...
	while(0!=fscanf(fp,"%[^\n]",buf)){
	 if(strlen(buf)+20>LOG_MAX)
		 break;
	 bzero(tmp,sizeof(tmp));
      sprintf(tmp,"<tr><td>%u</td><td>",index);
	 strcat(log,tmp);
	 strcat(log,buf);
	 strcat(log,"</td></tr>");
	 if('\n'!=getc(fp))
...
void formDefineManagement(void)
{
	...
	websFormDefine(T("getSystemLoading"), getSystemLoading);
	...
	websFormDefine(T("reboot"), reboot);

	websFormDefine(T("syslog"), syslog);
	websFormDefine(T("hertsyslog"), hertsyslog);
#if defined CONFIG_LOGREAD && defined CONFIG_KLOGD
