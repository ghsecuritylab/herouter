 ...
 *
 * $Id: //WIFI_SOC/MP/SDK_4_2_0_0/RT288x_SDK/source/user/goahead/src/sockGen.c#1 $
 */
...
int socketOpenConnection(char *host, int port, socketAccept_t accept, int flags, int ipv6_use)
{
...
//qiangzhenting add for get client IP	
#ifdef DARE_CUSTOMER_WEB
	char *getipaddr = NULL;
	if(strcmp(nvram_bufget(RT2860_NVRAM, "ClientIP"),inet_ntoa(addr.sin_addr))!=0 )
	{	
		getipaddr = inet_ntoa(addr.sin_addr);
		nvram_bufset(RT2860_NVRAM, "ClientIP", getipaddr);
