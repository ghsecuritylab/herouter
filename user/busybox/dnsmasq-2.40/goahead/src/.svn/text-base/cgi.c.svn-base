 ...
 *
 * $Id: //WIFI_SOC/MP/SDK_4_2_0_0/RT288x_SDK/source/user/goahead/src/cgi.c#1 $
 */
...
extern int g_upgradeState; /* 0: no process, 1: upgrading. */
		...
		char_t *url, char_t *path, char_t* query)
{
    ...
    printf("-----------wp->url(%s)-----------\n", wp->url);
    /* add for auto upgrade from upgrade file server via CTC platform, start */
    if(strstr(wp->url, "upload.cgiauto"))
    {
        int     bDoNext = 1;
        char    szCmd[128];
        /* let herouteapp to download the firmware from upgarde file server¡¡*/
        printf("let herouteapp to download the firmware from upgarde file server\n");
        ...
        printf("bDoNext(%d), wait for herouteapp to download the firmware from upgarde file server\n", bDoNext);
