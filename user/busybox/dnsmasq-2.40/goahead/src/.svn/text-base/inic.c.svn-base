 ...
 *
 *	$Id: //WIFI_SOC/MP/SDK_4_2_0_0/RT288x_SDK/source/user/goahead/src/inic.c#2 $
 */
...
#if	defined (CONFIG_SECOND_IF_MT7612E)
#define MAX_NUMBER_OF_MAC               116
#elif defined (RTDEV_WAPI_SUPPORT)
...
static void inicBasic(webs_t wp, char_t *path, char_t *query)
{
	...
	char_t	*n_mode, *n_bandwidth, *n_gi, *n_mcs, *n_rdg, *n_extcha, *n_amsdu, *n_stbc, *n_ldpc;
...
//#WPS
	...
	if (is_n | is_ac) {
		...
		nvram_bufset(RTDEV_NVRAM, "HT_LDPC", n_ldpc);
	}
	if (is_ac) {
		nvram_bufset(RTDEV_NVRAM, "VHT_BW", vht_bandwidth);
		nvram_bufset(RTDEV_NVRAM, "VHT_STBC", vht_stbc);
		nvram_bufset(RTDEV_NVRAM, "VHT_SGI", vht_sgi);
		nvram_bufset(RTDEV_NVRAM, "VHT_BW_SIGNAL", vht_bw_signal);
		nvram_bufset(RTDEV_NVRAM, "VHT_LDPC", vht_ldpc);
	}
...
static void inicAdvanced(webs_t wp, char_t *path, char_t *query)
{
	...
	char_t *wmm_capable, *apsd_capable, *dls_capable, *countrycode, *countryregion;
