 ...
 *
 * $Id: //WIFI_SOC/MP/SDK_4_2_0_0/RT288x_SDK/source/user/goahead/src/wsIntrn.h#1 $
 */
...
typedef struct tagNetworkStatus
{
    char wanPortStatus[8];
    char lan1PortStatus[8];
    char lan2PortStatus[8];
    char WanIpStatus[8];
    char pingStatus[8];
    char PPPOEStatus[8];
...
extern int 		 websCgiHandler(webs_t wp, char_t *urlPrefix, char_t *webDir,
					int arg, char_t *url, char_t *path, char_t *query);
extern int 		 websHertAppHandler(webs_t wp);
...
#ifdef DARE_CUSTOMER_WEB
extern int isFreePages(const char *page);
extern void quickSecurity(int nvram, webs_t wp, char_t *path, char_t *query);
extern void quickConfWEP(int nvram, webs_t wp, int mbssid);
extern void quickConfWPAGeneral(int nvram, webs_t wp, int mbssid);
