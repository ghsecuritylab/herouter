 ...
 *
 *	$Id: //WIFI_SOC/MP/SDK_4_2_0_0/RT288x_SDK/source/user/goahead/src/utils.c#2 $
 */
#include	<time.h>
#include	<signal.h>
#include	<sys/ioctl.h>
...
static int  getStbid(int eid, webs_t wp, int argc, char_t **argv);
...
static int  getNeedUpgradeTitle(int eid, webs_t wp, int argc, char_t **argv);
...
void formDefineUtilities(void)
{
	...
	websAspDefine(T("getStbid"), getStbid);
	...
	websAspDefine(T("getNeedUpgradeTitle"), getNeedUpgradeTitle);
}
...
static int getCfgZero(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if (1 == type) {
		if (!strcmp(value, ""))
		{
			if( (!strcmp(field, "Key1Str1")) || (!strcmp(field, "WPAPSK1")))
			{
				return websWrite(wp, T(""));
			}
			else
...
#else
    extern char* hertUtil_getFirmwareVersion();

    sprintf(version, "%s", hertUtil_getFirmwareVersion());
#endif
...
/*
 * description: write need upgrade
 */
static int getNeedUpgradeTitle(int eid, webs_t wp, int argc, char_t **argv)
{
	char version[32] = "";
...
static int getSysUptime(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	usecs = getUptime();
	utime.tm_hour = usecs/3600;
	utime.tm_min = (usecs%3600)/60;
	utime.tm_sec = (usecs%3600)%60;
...
static int getStbid(int eid, webs_t wp, int argc, char_t **argv)
{
    ...
    {
        printf("Failed to hertUtil_readMac\n");
        return 0;
    }
    sprintf(szSTBID, "%s%02X%02X%02X%02X%02X%02X", hertUtil_getSN(),
		0xff & if_mac[0], 0xff & if_mac[1],0xff & if_mac[2],0xff & if_mac[3], 0xff & if_mac[4], 0xff & if_mac[5]);
    printf("szSTBID = %s\n", szSTBID);
	return websWrite(wp, T("%s"), szSTBID);
}
