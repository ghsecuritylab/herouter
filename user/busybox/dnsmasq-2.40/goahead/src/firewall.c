 ...
 *
 *	$Id: //WIFI_SOC/MP/SDK_4_2_0_0/RT288x_SDK/source/user/goahead/src/firewall.c#2 $
 */
...
{
	int type, value;
	char *pfe = (char *)nvram_bufget(RT2860_NVRAM, "PortTriggerEnable");
	if(pfe)
		value = atoi(pfe);
	else
		value = 0;
...
{
	doSystem("iptables -t filter -D FORWARD -j %s 1>/dev/null 2>&1", PORT_TRIGGER_CHAIN);
	doSystem("iptables -t filter -F %s 1>/dev/null 2>&1", PORT_TRIGGER_CHAIN);
...
#ifdef CONFIG_RALINKAPP_SUPERDMZ
...
static void iptablesPortTriggerFlush(void){
    doSystem("iptables -t filter -F %s 1>/dev/null 2>&1", PORT_TRIGGER_CHAIN);
	doSystem("iptables -t nat -F %s 1>/dev/null 2>&1", PORT_TRIGGER_PREROUTING_CHAIN);
}
...
static void iptablesNATPortTriggerClear(void){
	doSystem("iptables -t nat -D PREROUTING -j %s 1>/dev/null 2>&1", PORT_TRIGGER_PREROUTING_CHAIN);
	doSystem("iptables -t nat -F %s  1>/dev/null 2>&1; iptables -t nat -X %s  1>/dev/null 2>&1", PORT_TRIGGER_PREROUTING_CHAIN, PORT_TRIGGER_PREROUTING_CHAIN);
}
...
static void iptablesPortTriggerRun(void)
{
	...
	while( (getNthValueSafe(i++, rule, ';', rec, sizeof(rec)) != -1) ){
		...
		if((getNthValueSafe(0, rec, ',', triggerProtocol, sizeof(triggerProtocol)) == -1))
			continue;
		triggerProto = atoi(triggerProtocol);
		// get trigger port
		if((getNthValueSafe(1, rec, ',', triggerPort, sizeof(triggerPort)) == -1)){
        ...
        if((getNthValueSafe(2, rec, ',', incomingProtocol, sizeof(incomingProtocol)) == -1))
			continue;
		incomingProto = atoi(incomingProtocol);
		// get incoming port
        if((getNthValueSafe(3, rec, ',', incomingPort, sizeof(incomingPort)) == -1)){
		...
		sprintf(cmd, "iptables -t nat -A %s -j TRIGGER --trigger-type dnat", PORT_TRIGGER_PREROUTING_CHAIN);
		doSystem(cmd);
}
...
/*set iptables for app set*/
static void iptablesForInteAccess(void)
{
    int nvramSize = 0,accCon = 0,netLimit = 1,multiFlag = 0;	
    char *ptr = NULL,*qtr = NULL,*pTmp = NULL,*pMult = NULL;
    char AccessRules[MAX_ACCESS_RULES_SIZE],currRules[64];
    char timeStart[8],timeEnd[8],timeStart2[8],timeEnd2[8],ipmac[32],szCmd[128];
        
    memset(AccessRules,0x0,MAX_ACCESS_RULES_SIZE);
    memset(currRules,0x0,sizeof(currRules));
    memset(timeStart,0x0,sizeof(timeStart));
    memset(timeEnd,0x0,sizeof(timeEnd));
    memset(timeStart2,0x0,sizeof(timeStart2));
    memset(timeEnd2,0x0,sizeof(timeEnd2));
    memset(ipmac,0x0,sizeof(ipmac));
    memset(szCmd,0x0,sizeof(szCmd));
    
    nvramSize = hertUtil_GetNvram_Values("InternetRules", AccessRules, MAX_ACCESS_RULES_SIZE);
    if(nvramSize > 0)
    { 
    	  ptr=AccessRules;
        while((*ptr != '\0') && ((qtr = strstr(ptr,",")) != NULL))
        {
        	  pTmp = currRules;
            strncpy(pTmp,ptr,(qtr - ptr));
            multiFlag = 0;
            //this rule include multi timr

            if((pMult = strstr(pTmp,INTERNET_TIME_SEPERATOR)) != NULL)
            {
                sscanf(pTmp, "%17s-%1d-%5s-%5s&%5s-%5s-%1d",ipmac,&accCon,timeStart,timeEnd,timeStart2,timeEnd2,&netLimit);
                multiFlag = 1;
            }
            else
            {
                sscanf(pTmp, "%17s-%1d-%5s-%5s-%1d",ipmac,&accCon,timeStart,timeEnd,&netLimit);
            }
            //not allow access internet
            if(accCon == 1)
            {
                sprintf(szCmd, "set_iptables.sh addDisable %s",ipmac);
                system(szCmd); 
                memset(szCmd,0x0,sizeof(szCmd));
                
                sprintf(szCmd, "set_iptables.sh add %s %s %s",ipmac,timeStart,timeEnd);
                system(szCmd); 
                if(multiFlag == 1)
                {
                    memset(szCmd,0x0,sizeof(szCmd));
                    sprintf(szCmd, "set_iptables.sh add %s %s %s",ipmac,timeStart2,timeEnd2);
                    system(szCmd); 
                }
            }
            else
...
static int showPortTriggerRulesASP(int eid, webs_t wp, int argc, char_t **argv)
{
    ...
    if((getNthValueSafe(3, rec, ',', incomingPort, sizeof(incomingPort)) == -1)){
     ...
     switch(triggerProto){
		...
		// output trigger Port 
			websWrite(wp, T("<td align=center> %d </td>"), triggerPort_int);
		...
		// output incoming Port 
			websWrite(wp, T("<td align=center> %d </td>"), incomingPort_int);
...
static void portTrigger(webs_t wp, char_t *path, char_t *query)
{
	...
	spfe = websGetVar(wp, T("portTriggerEnabled"), T(""));
	triggerPort = websGetVar(wp, T("triggerPortNumber"), T(""));
	incomingPort = websGetVar(wp, T("incomingPortNumber"), T(""));
	triggerProtocol = websGetVar(wp, T("triggerPortProtocol"), T(""));
	incomingProtocol = websGetVar(wp, T("incomingPortProtocol"), T(""));
	comment = websGetVar(wp, T("comment"), T(""));
...
void firewall_init(void)
{
	...
	doSystem("iptables -t nat -N %s 1>/dev/null 2>&1", PORT_TRIGGER_PREROUTING_CHAIN);
	doSystem("iptables -t nat -A PREROUTING -j %s 1>/dev/null 2>&1", PORT_TRIGGER_PREROUTING_CHAIN);
	...
	doSystem("iptables -I malicious_input_filter   -p udp --dport 53 -j ACCEPT ");
	iptablesAllNATRun();
	
#if 0	
	iptablesForInteAccess();
#endif
