...
/*
    add for change version foramt, start

VERSION: HE_ROUTE_VER=
    00-1411-9-001-R619

STBID: HE_ROUTE_SN=
    86999900000014060000
    86777700000014060000
    86320800000014060000
...
=>
    ...
    1411-3-208-R619
*/
const char *nvram_get_hertversion(int index, char *pszOriginVersion)
{
    static char szRetVer[64] = { 0x0 };
    char szSTBID[128] = { 0x0 };
    char *ptr = NULL;
    char *ptStbid = NULL;
    ...
    /* ======== format version =======
            1411-9-001-R619
    */
    memset(szRetVer, 0x0, sizeof(szRetVer));
    sprintf(szRetVer, "%s", pszOriginVersion);
    ...
    /* ======== get STBID =======
            ...
            86320800000014060000
    */
    memset(szSTBID, 0x0, sizeof(szSTBID));
#ifdef CONFIG_DUAL_IMAGE
    sprintf(szSTBID, "%s", nvram_bufget(UBOOT_NVRAM, "HE_ROUTE_SN"));
#endif
    if (strlen(szSTBID) <= 19 )
    {
        sprintf(szSTBID, "%s", nvram_bufget(index, "HE_ROUTE_SN"));
    }
    if (strlen(szSTBID) <= 19 )
    {
        printf("[%s,%d]: Invalid STBID format(%s)\n", __FUNCTION__, __LINE__, szSTBID);
        return "";
    }
    ptStbid = szSTBID;

    ptr= szRetVer;
    ptr += 5;    /* jump to, 9-001-R619 */
    ptStbid += 2; /* jump over country code, 999900000014060000 */
    *ptr = *ptStbid;

    ptr += 2;    /* jump to, 001-R619 */
    ptStbid += 1; /* jump over country code, 99900000014060000 */
    *ptr = *ptStbid;

    ptr += 1;    /* jump to, 01-R619 */
    ptStbid += 1; /* jump over country code, 9900000014060000 */
    *ptr = *ptStbid;

    ptr += 1;    /* jump to, 1-R619 */
    ptStbid += 1; /* jump over country code, 900000014060000 */
    *ptr = *ptStbid;

    //printf("[%s,%d]: Return the version(%s)\n", __FUNCTION__, __LINE__, szRetVer);
    return szRetVer;
}
...
char const *nvram_bufget_mtk(int index, char *name)
{
	int idx;
...
{
    /* ======== add for change version foramt, start ======= */
    if(0 ==strcmp(name, "HE_ROUTE_VER"))
    {
        char szOldVer[64] = { 0x0 };
        memset(szOldVer, 0x0, sizeof(szOldVer));
        sprintf(szOldVer, "%s", nvram_bufget_mtk(index, name));

        //printf("[%s,%d]:name(%s), szOldVer(%s)______\n", __FUNCTION__, __LINE__, name, szOldVer);
        return nvram_get_hertversion(index, szOldVer);
    }
    /* ======== add for change version foramt, end ======= */

    return nvram_bufget_mtk(index, name);
}
...
int nvram_bufset(int index, char *name, char *value)
{
	...
	printf("[%s,%d]:______ name(%s), value(%s), index(%d) ______\n", __FUNCTION__, __LINE__, name, value, index);
